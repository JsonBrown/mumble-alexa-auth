...
function SetDeviceInfo(key, device) {
	db.serialize(function() {
		if(!dbExists) {
			db.run("CREATE TABLE Devices (key VARCHAR(50) PRIMARY KEY, deviceSecret VARCHAR(255), accessToken VARCHAR(255), refreshToken VARCHAR(255), tokenExpires INT )");
			dbExists = true;
		}
		db.run(`INSERT OR REPLACE INTO Devices (key, deviceSecret, accessToken, refreshToken, tokenExpires) VALUES ('${key}', '${device.deviceSecret}', '${device.tokens.access}', '${device.tokens.refresh}', ${device.tokens.expires})`);
	});
}
function GetDeviceInfo() {
	var deviceInfo = {};
	if (dbExists) {
		db.each("SELECT key, deviceSecret, accessToken, refreshToken, tokenExpires FROM Devices", function(err, row) {
			deviceInfo[row.key] = {
			  deviceSecret: row.deviceSecret,
			  tokens: {
				access: row.accessToken,
				refresh: row.refreshToken,
				expires: row.tokenExpires,
			  }
			};
		});
	}
      ...
      var req = https.request(options, function (res) {
        ...
        res.on('end', function () {
          ...
          if (res.statusCode === 200 && resultBuffer !== null) {
			...
			SetDeviceInfo(key, deviceInformation[key]);
      ...
      var req = https.request(options, function (res) {
        ...
        res.on('end', function () {
          if (res.statusCode === 200 && resultBuffer !== null) {
			...
			SetDeviceInfo(productDsn, deviceInformation[productDsn]);
